...
- **Tabs** — a **TABS** pill marks every machining tab: on xcs this comes directly from the file's own `SetAttribute("TAB", ...)` calls; on G-code it's inferred from the path lifting 0–0.3mm for roughly 15–90mm of travel before dropping back down (a normal full-height retract isn't flagged).
